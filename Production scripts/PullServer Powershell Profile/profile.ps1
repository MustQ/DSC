{$CieDSCCustomFolder = Test-Path C:\CieDSCCustom
if ($CieDSCCustomFolder -eq $True) {set-location C:\CieDSCCustom}
Else {Set-Location c:\}}

# set environment to add custom module path:

#Save the current value in the $p variable.
$p = [Environment]::GetEnvironmentVariable("PSModulePath")

#Add the new path to the $p variable. Begin with a semi-colon separator.
$p += ";C:\CieDSCCustom\Making Resources\CieCustomComposite\WindowsPowershell\Modules"
...
Function New-DscCompositeResource
{
    ...
    param(
        ...
        [string]
        $Path = "C:\CieDSCCustom\Making Resources\CieCustomComposite\WindowsPowershell\Modules",
        [Parameter(Mandatory)]
        [ValidateNotNullOrEmpty()]
        [string]
        $ModuleName,
